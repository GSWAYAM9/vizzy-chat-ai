...
// Initialize Groq client safely - only if API key exists
...
if (process.env.GROQ_API_KEY) {
  try {
    ...
    console.warn("[v0] Failed to initialize Groq client")
  }
}
...
async function refinePromptWithGroq(userPrompt: string): Promise<string> {
  try {
    if (!groq) {
      console.log("[v0] Groq not available, using original prompt")
      return userPrompt
    }
